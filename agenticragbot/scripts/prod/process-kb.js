/**
 * scripts/prod/process-kb.js  — DEPRECATED THIN WRAPPER
 * ─────────────────────────────────────────────────────────────────────────────
 * Batch-processes *.md files from an input directory. This previously invoked a
 * standalone (drifted) copy of clean-and-chunk via execSync; it now copies the
 * inputs into the CLI workspace and delegates to `cli pipeline run --all`, which
 * uses src/pipeline/ as the single implementation. See cli/plan.md.
 *
 * USAGE:
 *   node scripts/prod/process-kb.js [--input kb/raw] [--output <ignored>] [--dry-run]
 *
...
const HERE = import.meta.dirname; // scripts/prod
